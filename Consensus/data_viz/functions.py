...
def calculate_sample(ga, df):
	if ga == 'Chinese':
		return f'Chinese (N={int(df["AC_C"].sum())})'
	elif ga == 'Indian':
		return f'Indian (N={int(df["AC_I"].sum())})'
	else:
		return f'Malay (N={int(df["AC_M"].sum())})'
...
def barplot_frequency(data, x_col, y_col, xlabel, ylabel, title, figsize=(30, 15), fontsize=15, label_fontsize=30, title_fontsize=None, x_ticks_rotation=True, ylim=None, legend=None, color='#ADD8E6', ci=None, colored_xticks=False, color_col=None, hue=None, palette='Set2', grid=False):
    ...
    chart = sns.barplot(x=x_col, y=y_col, data=data, color=color, hue=hue, palette=palette, edgecolor='black')
...
def donut_chart(ax, data, title, title_fontsize, startangle, label_color_map):
    ...
    ax.pie(x=data, autopct='%.2f%%', labels=data.keys(), pctdistance=0.5, startangle=startangle, colors=label_color_map, textprops={'fontsize': 12, 'verticalalignment': 'center','rotation': 0,}, )
